...
上記では、タスクスケジューラがデータベースを介して、環境側に配置されたセンサからの情報を受け取り、その値に応じて建設機械を動作させる概要図が示されている。
大まかな動作の流れはこちらで説明したとおりである。
本章では特に上図の赤枠で囲った、ROS2-TMS for Constructionが環境配置型センサからの情報を受け取る部分について説明する。
なお、本機能は以下のモジュール図の赤枠で囲ったTMS_SPに相当する部分である。



TMS_SPは「センシングシステム及びROS2-TMS for Costruction間通信のデータ型」・「受信したデータをROS2-TMS for Constrctionのデータベースに書き込むプログラム」
から構成される。従来これらの機能はこちらのROS2-TMS for Constructionのリポジトリ内で管理されていたものの、センシングシステムの仕様に依存している一面もあり、
こちらのパッケージに分けることとした。
...
3. そしてMongoDB Compassを起動し、~/rostmsdb/parameter下の以下のデータを確認すると、値が更新されていく様子が確認できる。
...
となる。なお、このようにタスク実行中にリアルタイムに更新されるデータベース上のデータのことを動的パラメータデータと呼んでいる。
